...
# separo o estado
uf.rj.geo <- brasil %>% filter(NM_ESTADO == "RIO DE JANEIRO")

# transformo para SIRGAS UTM, fuso 23S
uf.rj.utm <- st_transform(uf.rj.geo, crs = 31983)
```
...
rodovias.rj <- rodovias %>% filter(UF == "RJ")
rodovias.rj <- st_transform(rodovias.rj, crs = 31983)
...
plot(st_geometry(uf.rj.utm), graticule = T, axes = T)
plot(st_geometry(rodovias.rj), add = T, col = "red")
...
ruas.rj <- ruas %>% select(3, 5)
ruas.rj <- st_transform(ruas.rj, crs = 31983)
plot(st_geometry(ruas.rj), axes = T)
```
...
```{r spatial_44}
ruas.rj <- ruas.rj[!str_detect(ruas.rj$NM_TXTMEMO, "RODOVIA"),]
ruas.rj
```
...
```{r spatial_45}
brasil.mun<- st_read("data/brasil_municipios.shp", options = "ENCODING=windows-1252" )
brasil.mun
```
...
```{r spatial_46}
rj.mun <- brasil.mun[which(brasil.mun$UF == "RIO DE JANEIRO"),]
rj.mun <- st_transform(rj.mun, crs = 31983)
rj.mun
```
...
mun.rj <- rj.mun[which(rj.mun$NOME_MUN == "RIO DE JANEIRO"),]

# plotar o município e as rodovias
plot(st_geometry(mun.rj), col="grey", graticule=T, axes=T, reset = F)
plot(st_geometry(rodovias.rj), add = T, col = "Red", lwd=3)
```
...
```{r spatial_48}
rio.munic <- brasil.mun %>% filter(UF == "RIO DE JANEIRO" & NOME_MUN == "RIO DE JANEIRO")
plot(st_geometry(rio.munic))
```
...
rodovias.mun.rj <- st_intersection(rodovias.rj, mun.rj)
head(rodovias.mun.rj)

# Verificação
plot(st_geometry(mun.rj), 
     col       = "grey", 
     graticule = T, 
     reset     = F)
plot(st_geometry(rodovias.mun.rj), 
     add = T, 
     col = "Red", 
     lwd = 3)
```
...
rodovias.mun.rj.buff <- st_buffer(rodovias.mun.rj, 200)

# junta feicoes numa só
rodovias.mun.rj.buff <- st_union(rodovias.mun.rj.buff)
...
plot(st_geometry(mun.rj), 
     ...
     graticule = T, 
     reset     = F)
plot(st_geometry(rodovias.mun.rj.buff), 
     add = T, 
     col = "Red")
```
...
# com [[1]]
intersecao <- st_intersects(rodovias.mun.rj.buff,ruas.rj)[[1]]

# criamos um novo objeto com as ruas de intersecção
ruas.corte <- ruas.rj[intersecao,]
ruas.corte
...
```{r spatial_52}
library(forcats)
unique(ruas.corte$NM_TXTMEMO) %>%  fct_drop %>% fct_count() %>% summarise(n = n())
```
...
```{r spatial_53}
as.factor(rodovias.mun.rj$CODIGO) %>% fct_drop() %>% summary(("CODIGO"))
```


```{r spatial_54}
BR.040 <- filter(rodovias.mun.rj, CODIGO == "BR-040")
print(paste0("Extensão BR-040 = ",round(sum(BR.040$EXT_TRECHO),2)))

BR.101 <- filter(rodovias.mun.rj, CODIGO == "BR-101")
print(paste0("Extensão BR-101 = ",round(sum(BR.101$EXT_TRECHO),2)))

BR.116 <- filter(rodovias.mun.rj, CODIGO == "BR-116")
print(paste0("Extensão BR-116 = ",round(sum(BR.116$EXT_TRECHO),2)))

BR.465 <- filter(rodovias.mun.rj, CODIGO == "BR-465")
print(paste0("Extensão BR-465 = ",round(sum(BR.465$EXT_TRECHO),2)))
...
plot(st_geometry(mun.rj), 
     ...
     cex.axis = 0.8, 
     reset    = F)
plot(st_geometry(BR.040["UF"]), add = T, col= "blue",   lwd = 2)
plot(st_geometry(BR.101["UF"]), add = T, col= "red",    lwd = 2)
plot(st_geometry(BR.116["UF"]), add = T, col= "green",  lwd = 2)
plot(st_geometry(BR.465["UF"]), add = T, col= "yellow", lwd = 2)
