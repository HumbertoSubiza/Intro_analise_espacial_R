...
```{r}
direc <- ("C:/Users/Administrador/Documents/R/Intro_analise_espacial_R/exercicios/data")
```
...
getwd()
nig_pts <- st_read(paste(direc,"nova_iguacu_pontos.shp",sep = "/"))
...
getwd()
nig_lin <- st_read(paste(direc,"nova_iguacu_linha.shp",sep = "/"))
...
nig_pol <- st_read(paste(direc,"nova_ig_mun_UTF.shp",sep = "/"), options = "ENCODING=UTF-8")
...
rj_mun <- st_read(paste(direc,"RJ_municip.shp",sep = "/"), options = "ENCODING=UTF-8" )
...
```{r map08}
(nig_MDE <- raster(paste(direc,"MDE_27444ne_v1.tif",sep = "/")))
class(nig_MDE)
paste(direc,"MDE_27444ne_v1.tif",sep = "/")
```
...
```{r map09}
nig_orto <- brick(paste(direc,"27444ne.tif",sep = "/"))
class(nig_orto)
paste(direc,"27444ne_v1.tif",sep = "/")
```
...
# em disco:
file.size("~/R/Intro_analise_espacial_R/exercicios/data/27444ne.tif")
```
...
esper_df <- read.csv("~/R/Intro_analise_espacial_R/exercicios/data/df_100.csv")
head(esper_df, 3)
```
...
st_write(esper_sf,                       # dados a gravar
         "~/R/Intro_analise_espacial_R/exercicios/data/esper_novo.csv",        # novo arquivo com caminho de gravação
         layer_options = "GEOMETRY=AS_XY", # opção para salvar a geometria
         delete_dsn=TRUE)               # se o nome já existe, apague o arquivo primeiro

esper_novo <- read.csv("~/R/Intro_analise_espacial_R/exercicios/data/esper_novo.csv") # ler o arquivo gravado
head(esper_novo)
```
